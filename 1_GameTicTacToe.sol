...
contract TicTacToe {
    ...
    Achievement achiveament;

    //Constructor
    constructor(address achievementContract){
        achiveament = Achievement(achievementContract);
    }
    ...
    function saveWinner (uint winner, uint idGame) private {
        if (winner != 0) {
            ...
            if (winGames[games[idGame].winner]== 5){
                achiveament.generate(games[idGame].winner);
            }

            bool availableSpace;
            for (uint x=1; x<4; x++){
                for (uint y=1; y<4; y++){
                    if (games[idGame].moves[x][y] == 0) availableSpace = true ;
                }
            }
            if (availableSpace) achiveament.generate(games[idGame].winner);
        }
    }
